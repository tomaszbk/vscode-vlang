...
export const BUNDLED_EXECUTABLE = path.join(
	EXTENSION_ROOT_DIR,
	"libs",
	"bin",
	BINARY_NAME
...
export async function getVlsPath(): Promise<string> {
	const build_vls = getWorkspaceConfig().get<boolean>("vls.autoBuild", false);
	try {
	...
	if (!build_vls) {
		...
		throw new Error(
			`VLS builds not yet available. Please enable "vls.autoBuild" in settings.`
		);
	}
	...
	try {
		...
		throw new Error(`Failed to build or install VLS: ${(err as Error).message}`);
	}
}
...
function receiveLauncherJsonData(
	cb: (payload: { error?: { code: number; message: string }; message: string }) => void
) {
